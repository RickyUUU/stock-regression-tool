...
def apply_sma(df, close_col, fast_period, slow_period, only_buy):
    df_res = df.copy()
...
def apply_rsi(df, close_col, rsi_period, oversold, overbought, only_buy):
    df_res = df.copy()
...
def apply_kd(df, close_col, high_col, low_col, kd_period, kd_buy, kd_sell, only_buy):
    df_res = df.copy()
